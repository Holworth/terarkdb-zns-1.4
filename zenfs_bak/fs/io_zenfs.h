...
class ZoneFile {
 ...
 private:
  ...
  // Record the size of bytes write at a specific timepoint
  void RecordWrite(uint64_t size);

  // Check the specific file type:
  bool IsSSTFile() const {
    return io_type_ == IOType::kFlushSST ||
           io_type_ == IOType::kCompactionOutputSST;
  }

  bool IsFlushSST() const {
    return io_type_ == IOType::kFlushSST;
  }

  bool IsBlobFile() const {
    return io_type_ == IOType::kFlushBlob ||
           io_type_ == IOType::kCompactionOutputBlob;
  }

  bool IsCompactionOutputBlob() const {
    return io_type_ == IOType::kCompactionOutputBlob;
  }

  bool IsFlushBlob() const {
    return io_type_ == IOType::kFlushBlob;
  }
