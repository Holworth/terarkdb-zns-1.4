...
DEFINE_int64(num_op, -1, "Number of operations (YCSB benchmarks only)");
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 private:
  ...
  Stats RunBenchmark(int n, Slice name,
    ...
    // Init YCSB generator
    // The number of operations is not designated
    if (FLAGS_num_op == -1) {
      FLAGS_num_op = FLAGS_num * 2;
    }

    const int64_t num_opts = FLAGS_num_op;
    ...
    for (int i = 0; i < n; i++) {
      ...
      // Set the YCSB generator of each thread located in each Thread arguments
